...
const login = TryCatch(async(req, res, next) => {
    ...
    if(!user) return next(new ErrorHandler("Invalid username/password", 404));

    const isMatch =  await compare(password, user.password);

    if(!isMatch) return next(new ErrorHandler("Invalid username/password", 404));
...
const logout = (req, res) => {
    return res.status(200).cookie(APP_TOKEN,"", {...cookieOption, maxAge: 0}).json({
        success: true,
...
const searchUser = TryCatch(async(req, res) => {
    ...
    //All Users who are not my friends or people with whom i have not chatted with
    const allUsersExceptMeAndFriends = await User.find({_id:{$nin: allUsersFromMyChat}, name:{ $regex:name, $options:"i"}});
...
const sendFriendRequest = TryCatch(async (req, res, next) => {
    ...
    const request = await Request.findOne({$or:[{sender:req.user, reciever:userId},{sender:userId, reciever:req.user},]});
