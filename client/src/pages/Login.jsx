...
import { VisullayHiddenInput } from "../components/styles/StyledComponents";
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Something went wrong login", {id:toastId})
    } finally{
      setIsLoading(false);
    }
  }
              ...
              <form style={{width:"100%", marginTop:"1rem"}} onSubmit={handleSignUp}>
                ...
                <Stack position={"relative"} width={"10rem"} margin={"auto"}>
                  ...
                  <IconButton sx={{position:"absolute", bottom:"0", right:"0", color:"white", bgcolor:"rgba(0,0,0,0.5)", ":hover":{bgcolor:"rgba(0,0,0,0.7)"}}} component="label">
                    <>
                      <CameraAltIcon />
                      <VisullayHiddenInput type="file" onChange={avatar.changeHandler}/>
                    </>
                  </IconButton>
                </Stack>
                {
                  avatar.error && <Typography m={"1rem"} width={"fit-content"} color={"error"} variant="caption" > {avatar.error}</Typography>
                }
